...
* Readying a Graph
We aim to construct a graph showing the numbers of function definitions of
various lengths in the Emacs lisp sources.

** Printing the Columns of a Graph
A first version of a working printing function follows:

#+begin_src emacs-lisp
  ;;; First version
  (defun column-of-graph (max-graph-height actual-height)
      "Return list of strings tha tis one column of a graph."
      (let ((insert-list nil)
...
The following code will test the function:
...
A more compelling, second version of the function follows:

#+begin_src emacs-lisp
  (defvar graph-symbol "*"
    "String used as symbol in a graph, usually as asterisk.")

  (defvar graph-blank " "
    "String to be used as blank in a graph, usually a blank space.
    `graph-blank' must be the same number of columns wide as
    `graph-symbol'")

  ;;; Second version of `column-of-graph'
  (defun column-of-graph (max-graph-height actual-height)
    "Return MAX-GRAPH-HEIGHT strings; ACTUAL-HEIGHT are graph-symbols.

  The graph-symbols are contiguous entries at the end of the list.
  The list will be inserted as one column of a graph.  The strings
  are either `graph-blank' or `graph-symbol'."
...
Here is the function definition for ‘graph-body-print’:

#+begin_src emacs-lisp
  (defun graph-body-print (numbers-list)
    "Print a bar graph of the NUMBERS-LIST.  The NUMBERS-LIST
    consists of the Y-axis values."
...
The function can be tested with a short list of numbers:
...
The same goal can be achieved recursively:
...
#+begin_src emacs-lisp
  ...
  (defun recursive-graph-body-print-internal
    ...
    "Print a bar graph.  Used within recursive-graph-body-print
    function."
...
An echanced version of the function that also prints axes for the graph can be
found in the section “[[info:eintr#Full Graph][A Graph with Labeled Axes]]” in the appendix of this book.
...
expressions in Emacs Lisp it is possible to change or extend Emacs.

** Specifying Variables using ‘defcustom’
The macro ‘defcustom’ declares its first argument, ~SYMBOL~, as a customizable
variable (it should not be quoted).  The following argument, ~STANDARD~, is an
expression specifying the varaible’s standard value.  It should not be quoted.
Following the documentation string are the optional keyword/value argument
pairs.  They should take the form ~[KEYWORD VALUE]…~

** Keymaps
Emacs uses “keymaps” to record which keys call which commands.  The command
‘global-set-key’ sets the keybinding for a single command in all parts of Emacs,
specifying the keybinding in ‘current-global-map’.

Mode-specific keymaps are bound using the ‘define-key’ function, which takes a
specific keymap as an argument, as well as the key and the command to be bound.

Customization of keybindings is detailed in the section “[[info:Emacs#Key Bindings][Customizing Key
Bindings]]” section of the Emacs manual.

*** Customizing Key Bindings
Emacs commands are Lisp functions whose definitions provide for interactive
use.  A “key sequence” (“key” for short) is a sequence of “input events” that
have a meaning as a unit.  Input events include characters, function keys, and
mouse buttons—all the forms of input a computer can receive.

Since most modes define their key bindings, activating a mode might override a
user’s custom key bindings.  A small number of keys are reserved for
user-defined bindings, and should, by convention, not be used by modes.  The
reserved key sequences are those consisting of the prefix ~C-c~ followed by a
letter (either upper of lower case), and function keys ~<F5>~ through ~<F9>~
without modifiers.
...
A “key sequence” (“key” for short) is a sequence of “input events” that have a
meaning as a unit.  Input events include characters, function keys, and mouse
buttons—all the forms of input a computer can receive.  A key sequence acquires
its meaning from its “binding”, which conveys the command that is invoked.  The
bindings between key sequences and command functions are recorded in data
...
are common to most or all major modes.  Each major or minor mode can have its
own keymap which overrides the global definitions of some keys.

**** Prefix Keymaps
Internally, Emacs records only single events in each keymap.  Interpreting a key
sequence of multiple events involves a chain of keymaps: the first keymap gives
a definition for the first event, which is another keymap; which is used to look
up the second event in the sequence, and so on.  Thus, a prefix key such as
~C-x~ or ~<ESC>~ has its own keymap, which holds the definition for the event
that immediately follows that prefix.

Thus, the binding of ~C-x~ is the symbol ‘Control-X-prefix’, whose function
definition is the keymap for ~C-x~ commands.  The definitions of ~C-c~, ~C-x~,
~C-h~, and ~<ESC>~ as prefix keys appear in the global map, so these prefix keys
...
+ ‘esc-map’ is for characters that follow ~<ESC>~.  Thus, all Meta characters
  are actually defined in this map.
+ ‘ctl-x-4-map’ is for characters that follow ~C-x 4~.
+ ‘mode-specific-map’ is for characters that follow ~C-c~.

**** Local Keymaps
Major modes customize Emacs by providing their own key bindings in “local
keymaps”.  Minor modes can also have local keymaps; whenever a minor mode is in
effect, the definitions in its keymap override both the major mode’s local
keymap and global keymap.  In addition, portions of text in the buffer can
specify their own keymaps, which override all other keymap.

Emacs handles a multi-event key sequence by looking in several keymaps, one by
one, for a binding of the whole key sequence.  First it checks the minor mode
keymaps for minor modes that are enabled, then it checks the major mode’s
keymap, and then it checks the global keymap.
...
To redefine an Emacs key, change its entry in a keymap.  In this section, we
describe how to rebind keys for the present Emacs session.

+ ~M-x global-set-key <RET> KEY CMD <RET>~ :: Define KEY globally to run CMD.
+ ~M-x local-set-key <RET> KEY CMD <RET>~ :: Define KEY locally (in the major
  mode in effect) to run CMD.
+ ~M-x global-unset-key <RET> KEY~ :: Make KEY undefined in the global map.
+ ~M-x local-unset-key <RET> KEY~ :: Make KEY undefined locally (in the major
  mode in effect).

**** Rebinding Keys in the Init File
In order to be available in every Emacs session, key bindings can be recorded in
the Emacs initialization file.  There are several ways to write a key binding
using Lisp.  The simplest is to use the ‘kbd’ function, which converts a textual
representation of a key sequence into a form that can be pased as an argument to
‘global-set-key’.  For example, here is how to bind ~C-z~ to the ‘shell’
command:
...
Here’s how to format key bindings for use with the ‘kbd’ function:

 + The special words ‘RET’, ‘SPC’, ‘TAB’, ‘DEL’, ‘LFD’, ‘ESC’ and ‘NUL’
   represent special control characters.  The words must be written in
   uppercase.
  
 + A word in angle brackets, e.g., <return>, <down>, or <f1>, represents a
   function key.  (Note that in the standard configuration, the function key
   <return> and the control key RET are synonymous.)  You can use angle brackets
   on the words RET, SPC, etc., but they are not required there.
 ...
 + One or more prefixes, ~M~- (meta), ~C-~ (control), ~S-~ (shift), ~A-~ (alt),
   ...
   ~C-<down>~ is the same as ~<C-down>~.  The prefixes may be written in any
   order: ~M-C-x~ is the same as ~C-M-x~.
   ...
   ~M--123~ is the same as ~M-- M-1 M-2 M-3~.
  
 + The ~^~ notation for control characters also works: ~^M~ is the same as
   ~C-m~.

 + Double angle brackets enclose command names: <<next-line>> is shorthand for
   ~M-x next-line RET~.
...
Instead of using ‘kbd’, one can use a Lisp string or vector to specify the key
sequence.  For example, here is how to bind ~C-x M-l~ to make-symbolic-link:

#+begin_src emacs-lisp
  (global-set-key "\C-x\M-l" 'make-symbolic-link)
#+end_src

To bind a sequence including <TAB>, <RET>, <ESC>, or <DEL>, the string should
contain the Emacs Lisp escape sequence ~\t~, ~\r~, ~\e~, or ~\d~, respectively.
Here is an example which binds ~C-x <TAB>~ to ‘indent-rigidly’:

#+begin_src emacs-lisp
  (global-set-key "\C-x\t" 'indent-rigidly)
#+end_src

To change keymaps defined by major and minor modes, one must use the “mode
hook”.  For example, Texinfo mode runs the hook ‘texinfo-mode-hook’; here is how
one can use the hook to add local bindings for ~C-c n~ and ~C-c p~ in Texinfo
mode:
